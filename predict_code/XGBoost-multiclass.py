...
def plot_confusion_matrices(y_true, y_pred, class_names):
    ...
    y_pred: 预测标签，形状为(n_samples, n_classes)
    class_names: 类别名称列表
    """
    # 创建figure目录（如果不存在）
    import os
    ...
    for i, cm in enumerate(cm_list):
        ...
        plt.title(f'Binary Confusion Matrix for {class_names[i]}')
        plt.colorbar()
        ...
        # 保存图像
        plt.savefig(f'figure/XGBoost_Binary_Confusion_Matrix_{class_names[i]}.png')
        plt.close()
    ...
    # 只考虑至少有一个类别为正的样本
    valid_samples = np.where(np.sum(y_true, axis=1) > 0)[0]
    
    if len(valid_samples) > 0:
        # 筛选有效样本
        y_true_filtered = y_true[valid_samples]
        y_pred_filtered = y_pred[valid_samples]
        
        # 将多标签转换为多类别表示
        y_true_multiclass = np.zeros(len(y_true_filtered))
        y_pred_multiclass = np.zeros(len(y_pred_filtered))
        
        for i in range(len(y_true_filtered)):
            # 找到真实标签中为1的第一个类别
            true_indices = np.where(y_true_filtered[i] == 1)[0]
            if len(true_indices) > 0:
                y_true_multiclass[i] = true_indices[0]
            
            # 找到预测标签中为1的第一个类别
            pred_indices = np.where(y_pred_filtered[i] == 1)[0]
            if len(pred_indices) > 0:
                y_pred_multiclass[i] = pred_indices[0]
        
        # 计算5×5混淆矩阵
        overall_cm = confusion_matrix(y_true_multiclass, y_pred_multiclass, 
        ...
        plt.title('5×5 Confusion Matrix')
        plt.xlabel('Predicted Label')
        plt.ylabel('True Label')
        plt.tight_layout()
        plt.savefig('figure/XGBoost_5x5_Confusion_Matrix.png')
        plt.show()
        pause()
        plt.close()
        
        # 计算并绘制归一化的混淆矩阵
        row_sums = overall_cm.sum(axis=1)
        cm_normalized = np.zeros_like(overall_cm, dtype=float)
        for i in range(len(row_sums)):
            if row_sums[i] > 0:
                cm_normalized[i] = overall_cm[i] / row_sums[i]
        
        plt.figure(figsize=(12, 10))
        sns.heatmap(cm_normalized, annot=True, fmt='.2f', cmap='Blues',
                   xticklabels=class_names, yticklabels=class_names)
        
        plt.title('Normalized 5×5 Confusion Matrix')
        plt.xlabel('Predicted Label')
        plt.ylabel('True Label')
        plt.tight_layout()
        plt.savefig('figure/XGBoost_5x5_Normalized_Confusion_Matrix.png')
        plt.close()
    else:
        print("警告：没有找到至少有一个类别为正的样本，无法绘制5×5混淆矩阵")
...
if __name__ == '__main__':
    ...
    for fold, (train_index, validation_index, test_indices) in enumerate(folds_data_index):
        ...
        # 绘制混淆矩阵
        # 首先需要获取预测标签
        preds = np.zeros_like(all_labels, dtype=np.float64)
        for i in range(all_labels.shape[1]):
            class_probs = all_probs[i][:, 1]
            preds[:, i] = (class_probs >= probs_switcher[i]).astype(np.float64)

        plot_confusion_matrices(all_labels, preds, columns_to_encode)
    ...
    if len(all_test_results) > 0:
        ...
        if len(all_test_results) > 0:
            # 收集所有测试数据的标签和预测
            all_test_labels = np.vstack([result['labels'] for result in all_test_results])
            
            # 计算所有测试数据的预测
            all_test_preds = []
            for idx, result in enumerate(all_test_results):
                probs = result['probs']
                switcher = all_probs_switchers[idx]
                preds = np.zeros_like(result['labels'], dtype=np.float64)
                for i in range(result['labels'].shape[1]):
                    class_probs = probs[i][:, 1]
                    preds[:, i] = (class_probs >= switcher[i]).astype(np.float64)
            ...
            # 绘制总混淆矩阵
            print("\n绘制所有测试数据的总混淆矩阵...")
            plot_confusion_matrices(all_test_labels, all_test_preds, columns_to_encode)            
    ...
    if len(selected_folds) == 5:
        ...
        for class_idx in range(len(columns_to_encode)):
            ...
            # 获取PCA特征的重要性
            pca_feature_importance = importance_df['importance'].values
            # 计算原始特征的重要性
            original_feature_importance = np.abs(pca_components.T) @ pca_feature_importance
            # 创建原始特征重要性DataFrame
            ...
            # 打印前20个重要的原始特征
            print(f"类别 {columns_to_encode[class_idx]} 的前20个重要原始特征:")
            print(original_importance_df.head(20))
            # 绘制原始特征重要性图
            plt.figure(figsize=(12, 8))
            top_n = min(20, len(original_importance_df))
            plt.barh(original_importance_df['feature'].head(top_n), original_importance_df['importance'].head(top_n))
            plt.xlabel('Importance')
            plt.ylabel('Original Feature')
            plt.title(f'Top {top_n} Original Feature Importance for {columns_to_encode[class_idx]}')
            plt.tight_layout()
            plt.savefig(f'figure/XGBoost_Original_Feature_Importance_{columns_to_encode[class_idx]}.png')

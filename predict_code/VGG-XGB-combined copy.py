...
class VGGFeatureExtractor(nn.Module):
    def __init__(self, vgg_model, input_dim=92):
...
# SHAP分析
def global_shap_analysis(models, background_data, test_data, feature_names, class_names, output_dir='figure/global_shap'):
    """
    执行全局SHAP分析并生成可视化图表
...
if __name__ == '__main__':  
    ...
    for fold, (train_index, validation_index, test_indices) in enumerate(folds_data_index):
        ...
        # 初始化VGG模型
        model = Model_VGG.VGG11(num_classes=num_classes, in_channels=features_pca.shape[1], epoch=num_epochs)
        model.to(device)
        ...
        for epoch in range(num_epochs):
            ...
            with torch.no_grad():
                for data, target in Validation_data_loader:
                    ...
                    probs = probs_sigmoid.cpu().numpy()
                    
                    # 计算阈值
                    probs_Switcher = Probs_Switcher(probs, target.cpu().numpy())
                    
                    # 计算指标
                    f1, accuracy, precision, recall, _, _ = f1_score_func(probs, target.cpu().numpy(), probs_Switcher)
            ...
            print(f'Epoch [{epoch + 1}/{num_epochs}]: Train Loss: {average_train_loss:.4f}, '
                  f'Validation F1: {valid_f1:.2f}%, Validation Accuracy: {valid_accuracy:.2f}%')
            ...
            if valid_f1 > best_valid_f1:
                ...
                best_epoch = epoch
                # 保存最佳模型
                torch.save(model.state_dict(), f'vgg_feature_extractor_fold{fold+1}.ckpt')
            else:
                counter += 1
        ...
        # 加载最佳模型
        model.load_state_dict(torch.load(f'vgg_feature_extractor_fold{fold+1}.ckpt'))
        model.eval()
        ...
        # 执行全局SHAP分析
        global_shap_analysis(xgb_models, train_features[:100], shap_data, xgb_feature_names, columns_to_encode)
